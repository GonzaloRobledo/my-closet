...
<h1>Editar Fotos  <?php echo htmlentities($category_display_title , ENT_QUOTES, "UTF-8");?></h1>

<p class="breadcrumb"><a href="/">Inicio</a> <?php if($gallery_url!=''){ ?>&gt; <a href="<?php echo $gallery_url;?>">Productos</a> <?php } ?>&gt; <a href="<?php echo $gallery_url;?>/<?php echo rawurlencode($category_title);?>"><?php echo htmlentities($category_display_title , ENT_QUOTES, "UTF-8");?></a> &gt; Editar Fotos</p>




<?php if(count($categories_array[$category_title])<=0){?>
<p>No hay fotos bajo <strong><?php echo htmlentities($category_display_title , ENT_QUOTES, "UTF-8");?></strong>.</p>
<?php } ?>
...
<p>En esta página puedes editar el título y la categoría de cada foto..</p>

<p>
Escriba un nuevo nombre de foto y presione Entrar o haga clic &quot;enviar&quot;.</p>
    ...
    <form id="form_<?php echo $photo_counter;?>" name="form_<?php echo $photo_counter;?>" method="get" action="" style="display:block; padding:10px; width:100%; height:<?php echo $settings_thumbnail_height;?>px; margin:0px; margin-left:-10px; border-top:1px solid #CCCCCC;  background-image:url('<?php echo $gallery_url."/files/".rawurlencode($category_title)."/".rawurlencode($photo_file)."_thumb.jpg" ;?>'); background-repeat:no-repeat; background-position:10px 10px;" onsubmit="return submit_edit_photo_form(<?php echo $photo_counter;?>);">
    	...
    	<span style="float:right; display:inline-block; width:150px; text-align:right;">
        	...
        	<a href="javascript:void(0);" id="make_cover_button_<?php echo $photo_counter;?>" onmouseup="make_cover(<?php echo $photo_counter;?>);" class="liquid_button" title="make this photo album cover">Establecer como Portada</a>
        	
			<a href="javascript:void(0);" onmouseup="confirm_delete_photo(<?php echo $photo_counter;?>);" class="liquid_button_red" title="delete this photo">Borrar</a>
		...
		<span style="display:inline-block; margin-left:<?php echo $settings_thumbnail_width+10;?>px;">
            ...
            <input type="submit" name="Submit" id="button_<?php echo $photo_counter;?>" value="Guardar" style="float:left; margin-right:10px; display:none;" class="button_110" />
